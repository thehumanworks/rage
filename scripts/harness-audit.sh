...
for script in scripts/qa.sh scripts/verify.sh scripts/smoke-local.sh scripts/smoke-infisical.sh scripts/harness-audit.sh; do
  [ -x "$script" ] || {
...
grep 'do not require.*gcloud' AGENTS.md README.md .codex/skills/rage-secrets/SKILL.md >/dev/null
...
grep 'smoke-infisical' docs/TESTING.md >/dev/null
...
if grep -R 'need age\|Command::new("age"' tests src .github scripts/qa.sh scripts/smoke-local.sh scripts/smoke-infisical.sh 2>/dev/null; then
  echo "found a required external age binary dependency" >&2
  exit 1
fi

if grep -R 'age-keygen' tests src .github scripts/qa.sh scripts/smoke-local.sh scripts/smoke-infisical.sh 2>/dev/null; then
  echo "found a required external age keygen dependency" >&2
  exit 1
fi

if grep -R 'need gcloud\|Command::new("gcloud"\|active `gcloud`\|Install gcloud' tests src .github scripts/qa.sh scripts/smoke-local.sh scripts/smoke-infisical.sh 2>/dev/null; then
