---
name: rage-secrets
description: Use when working inside the rage repository, a Rust CLI for GCP Secret Manager backed personal secrets with age-encrypted local cache, shell/exec/ssh loading, agent auth runners, and explicit macOS Keychain SSH gating.
---
...
- Keep GCP Secret Manager access native through HTTPS; do not require the `gcloud` CLI.
...
- Keep `cargo test` deterministic. Use the fake GCP Secret Manager server and fake external tools there.
- Use `scripts/smoke-gcp.sh` only for explicit live GCP verification.
...
- GCP request behavior: add `scripts/smoke-gcp.sh` or `RAGE_LIVE_GCP=1 scripts/verify.sh`
