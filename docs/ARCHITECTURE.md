...
- **Agent auth runner**: `rage import grok`, `rage import codex`, `rage grok`, and `rage codex` live in `src/agent_auth.rs`. Imported agent auth records are stored in Infisical root secrets named `AUTHLESS_<PROVIDER>_JSON`, refreshed through the provider OAuth endpoints when stale, and injected only into the intended child environment or managed auth file. Bundle operations reserve those keys and never sync them into shell caches.
